...
const ScheduleEmailSmsLog = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      const response = await axios.get('/api/communicate/schedule_email_sms_logs');
  ...
  return (
    <Container maxWidth="lg" style={{ padding: '20px 40px' }}>
      <Box mt={4} mb={4}>
        ...
        <AiFillSchedule style={{ marginRight: '5px', verticalAlign: 'middle', color: 'orange', fontSize: '33px', marginTop: '-7px' }} />Schedule Email SMS Log
        </Typography>
        <Paper elevation={3} style={{ padding: '30px 40px', borderRadius: '8px' }}>
          ...
          />
          <TableContainer className='scheduleemailcontain'>
            <Table className="scheduleemailtable">
              <TableHead>
                <TableRow>
              ...
              <TableBody>
                {filteredLogs.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((log) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>{log.schedule_date}</TableCell>
